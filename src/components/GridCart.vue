<template>
  <div class="cart">
    <h1 class="title">Your Basket - GRID</h1>
    <sup>
    ...
    <div class="grid main" v-for="p in products" track-by="id" :key="p.id">
      <template v-if="p.quantity > 0">
        ...
        <span class="right"> £{{ (p.quantity * p.price).toFixed(2) }} </span>
        <span class="blank">
    ...
    <div class="grid">
      ...
      <span></span>
      <span></span>
      <span><strong>£{{ (total + total * 0.2).toFixed(2) }}</strong></span>
      <span></span>
    </div>

    <button :disabled="total == 0" class="checkout is-grid right " @click="Checkout">
      Buy Now >>
    </button>
  </div>
</template>
